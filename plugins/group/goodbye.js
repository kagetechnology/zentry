...
let handler = async (m, { conn, args, rawText, downloadMediaMessage }) => {
  ...
  const saveImage = async (targetMsg, prefix) => {
    ...
    const filePath = path.join(dir, `goodbye_${prefix}_${safeJid}.jpg`)
  ...
  switch (subCmd) {
    case 'on':
      ...
      return m.reply('🟢 Goodbye *on*')

    case 'off':
      dbSet(`${key}.enabled`, false)
      return m.reply('⚫ Goodbye *off*')
    ...
    case 'setbg': {
      ...
      if (!target) return m.reply('❌ Kirim/reply gambar dengan caption *.goodbye setbg*')
      try {
    ...
    case 'setimg': {
      const target = getImageTarget()
      if (!target) return m.reply('❌ Kirim/reply gambar dengan caption *.goodbye setimg*')
      try {
    ...
    case 'show': {
      const text = (dbGet(`${key}.text`, DEFAULT_GOODBYE) || '').replace(/\\n/g, '\n')
      const bgPath = dbGet(`${key}.background`, null)
      const imgPath = dbGet(`${key}.image`, null)
